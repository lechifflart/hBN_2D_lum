...
def matdyn_freq_parser(file):
    ...
    for i in range(1,len(lines)-step,step+1):
        ...
        #print(np.ravel(np.array([line.split() for line in lines[i+1:i+step+1]],dtype=object)))
        frequencies[q_index] = np.array([float(x) for x in np.ravel([line.split() for line in lines[i+1:i+step+1]])])
...
#matdyn_freq_parser('another_matdyn_output_test')
